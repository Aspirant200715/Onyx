...
impl PathMatcher {
    pub fn matches(route: &str, request: &str) -> bool {
        ...
        let request_segments: Vec<&str> = request.trim_matches('/').split('/').collect();
...
mod tests {
    ...
    fn matches_dynamic_route() {
        assert!(PathMatcher::matches("/users/:id", "/users/42",));
    }
    ...
    fn rejects_wrong_segment_count() {
        assert!(!PathMatcher::matches("/users/:id", "/users",));
    }

    #[test]
    fn rejects_wrong_static_segment() {
        assert!(!PathMatcher::matches("/users/:id", "/posts/42",));
    }

    #[test]
    fn matches_nested_static_route() {
        assert!(PathMatcher::matches("/users/profile", "/users/profile",));
    }
}

#[test]
fn extracts_single_parameter() {
    let params = PathMatcher::extract_params("/users/:id", "/users/42");
...
fn extracts_multiple_parameters() {
    let params = PathMatcher::extract_params("/users/:user/posts/:post", "/users/7/posts/99");

    assert_eq!(params.get("user"), Some(&"7".to_string()));

    assert_eq!(params.get("post"), Some(&"99".to_string()));
}
